...
//   const displayName = prompt.name || prompt.user?.username || 'Anonymous';

//   const handleLike = async () => {
//     try {
//       // Toggling the like state
//       const response = await axios.post(
//         `/api/prompts/${prompt._id}/like`,
//         {},
//         {
//           headers: {
...
//       // Update the prompt state to reflect the new like status
//       setPrompts((prevPrompts) =>
//         prevPrompts.map((p) =>
//           p._id === prompt._id
//             ? { ...p, likes: response.data.likes, isLiked: response.data.isLiked }
//             : p
//         )
//       );
...
//         prev ? prev.map((p) =>
//           p._id === prompt._id
//             ? { ...p, isLiked: response.data.isLiked, likes: response.data.likes }
//             : p
//         ) : prev
//       );
...
//     } catch (err) {
//       console.error('Failed to toggle like for the prompt:', err.response?.data || err.message);
//     }
//   };
...
//     try {
//       const response = await axios.post(
//         `/api/prompts/${prompt._id}/bookmark`,
//         {},
//         {
//           headers: {
//             Authorization: `Bearer ${token}`,
...
//         prevPrompts.map((p) =>
//           p._id === prompt._id
//             ? { ...p, isBookmarked: response.data.isBookmarked }
//             : p
//         )
//       );
...
//       setBookmarkedPrompts((prev) => [
//         ...prev,
//         { ...prev.find((p) => p._id === prompt._id), isBookmarked: true },
//       ]);
//     } else {
//       // Remove from bookmarks if unbookmarked
//       setBookmarkedPrompts((prev) =>
//         prev.filter((p) => p._id !== prompt._id)
//       );
//     }
  
//     } catch (err) {
//       console.error('Failed to toggle bookmark for the prompt:', err.response?.data || err.message);
//     }
//   };
...
// console.log(prompt?.user?.totalPrompts);
//   return (
...
//                     {prompt?.user?.totalPrompts ? getPromptRank(prompt.user.totalPrompts) : 'User Not Found'}
//                   </span>
//               </div>
//       </div>
//           <div className="flex items-center gap-2">
